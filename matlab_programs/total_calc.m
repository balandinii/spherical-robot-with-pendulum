...
phi=equation_9( q, p, gamma, omega, e );
psi=equation_10( q, p, gamma, omega, e );
alpha=equation_11( J, mu );
beta=equation_12( j );
[phi_c, phi_l] = decompose(phi);
[psi_c, psi_l] = decompose(psi);
[e_c, e_l] = decompose(e);
...
if (abs(e_l) < 1.0e-16)
    ...
    t_omega_c=equation_28_29( c, e_c, a, b, gamma );
...
t_w(3)=-t_w_l;
t_omega=omega;
t_omega(1)=t_omega_c(1);
t_omega(2)=t_omega_c(2);
t_omega(3)=-t_omega_l;
t_e=vp(omega,e);
t_xi=vp(gamma, w);
